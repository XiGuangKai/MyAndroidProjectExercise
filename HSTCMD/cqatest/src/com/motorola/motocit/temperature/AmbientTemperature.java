...
public class AmbientTemperature extends Test_Base implements SensorEventListener
{
    private SensorManager mSensorManager;
    double fAmbientTemperature = -9999;
    ...
    {
        TAG = "Sensor_Ambient_Temperature";

        super.onCreate(savedInstanceState);

        mSensorManager = (SensorManager) this.getSystemService(SENSOR_SERVICE);

        View thisView = adjustViewDisplayArea(com.motorola.motocit.R.layout.ambient_temperature);
        if (mGestureListener != null)
        {
            thisView.setOnTouchListener(mGestureListener);
        }

        AxisText = (TextView) findViewById(com.motorola.motocit.R.id.ambient_temperature);

        if (Build.VERSION.SDK_INT < 14)
        {
            // TYPE_TEMPERATURE has been deprecated since API Level 14
            temperatureSensor = Sensor.TYPE_TEMPERATURE;
        }
        else
        {
            // API Level 14 and later uses TYPE_AMBIENT_TEMPERATURE
    ...
    protected void onResume()
    {
        ...
        if (sensorListenerRegistered == false)
        {
            ...
            if (wasActivityStartedByCommServer())
            {
                ...
                mSensorManager.registerListener(this, mSensorManager.getDefaultSensor(temperatureSensor), SensorManager.SENSOR_DELAY_FASTEST);
            }
            else
            {
                activityStartedFromCommServer = false;
                dbgLog(TAG, "activity originated UI .. setting update rate to UI rate", 'd');
                mSensorManager.registerListener(this, mSensorManager.getDefaultSensor(temperatureSensor), SensorManager.SENSOR_DELAY_UI);
            }
    ...
    {
        if (event.sensor.getType() == temperatureSensor)
        {
            fAmbientTemperature = event.values[0];
            ...
            if ((System.currentTimeMillis() - lastUiUpdateTime) > UI_UPDATE_RATE)
            {
                ...
                AxisText.setText("Temperature:" + TestUtils.round(fAmbientTemperature, PRECISION));

                dbgLog(TAG, "sensor: ambient temperature " + "Ambient Temperature=" + fAmbientTemperature, 'd');
            }
        }
    }
    ...
    protected void handleTestSpecificActions() throws CmdFailException, CmdPassException
    {
        ...
        if (strRxCmd.equalsIgnoreCase("GET_READING"))
        {
            ...
            strDataList.add(String.format("AMBIENT_TEMPERATURE=" + fAmbientTemperature));
        ...
        else if (strRxCmd.equalsIgnoreCase("GET_SENSOR_INFO"))
        {
            ...
            Sensor sensorObject = mSensorManager.getDefaultSensor(temperatureSensor);
    ...
    protected void printHelp()
    {
        ...
        strHelpList.add(TAG);
        strHelpList.add("");
        strHelpList.add("This function will read the Ambient Temperature");
        strHelpList.add("");
        ...
        strHelpList.add("  ");
        strHelpList.add("GET_READING - Returns Ambient Temperature in degrees Celsius");
        strHelpList.add("  ");
    ...
    public boolean onKeyDown(int keyCode, KeyEvent ev)
    {
        ...
        if ((keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) || (keyCode == KeyEvent.KEYCODE_VOLUME_UP))
        {
            ...
            {
                contentRecord("testresult.txt", "Ambient Temperature Test: PASS" + "\r\n", MODE_APPEND);

                logResults(TEST_PASS);
            }
            else
            {
                contentRecord("testresult.txt", "Ambient Temperature Test: FAILED" + "\r\n", MODE_APPEND);

                logResults(TEST_FAIL);
            }

            contentRecord("testresult.txt", "Ambient Temperature: " + String.valueOf(fAmbientTemperature) + "\r\n\r\n", MODE_APPEND);
    ...
    private void logResults(String passFail)
    {
        ...
        testResultName.add("AMBIENT_TEMPERATURE");

        testResultValues.add(String.valueOf(fAmbientTemperature));
