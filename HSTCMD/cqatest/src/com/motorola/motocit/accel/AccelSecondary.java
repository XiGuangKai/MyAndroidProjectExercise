...
public class AccelSecondary extends Test_Base implements SensorEventListener {
    ...
    private int secondAccelId;
    private boolean isSupported = false;

    @Override
    public void onCreate(Bundle savedInstanceState)
    {
        TAG = "Sensor_AccelerometerSecondary";
        super.onCreate(savedInstanceState);

        mSensorManager = (SensorManager) getSystemService(SENSOR_SERVICE);

        View thisView = adjustViewDisplayArea(com.motorola.motocit.R.layout.accel2nd);
    ...
    protected void onResume()
    {
        ...
        if (sensorListenerRegistered == false)
        {
            ...
            List<Sensor> mSensorList = mSensorManager.getSensorList(Sensor.TYPE_ACCELEROMETER);
            Sensor tmpSensor;
            int i = 0;
            if (mSensorList.size() > 0)
            {
                for (i = 0; i < mSensorList.size(); i++)
                {
                    ...
                    if (tmpSensor.getName().contains("Secondary"))
                    {
                        secondAccelId = i;
                        isSupported = true;
                    }
            ...
            {
                Toast.makeText(AccelSecondary.this, "The device does NOT support Accel sensor", Toast.LENGTH_SHORT).show();
                AccelSecondary.this.finish();
            }

            if (!isSupported)
            {
                Toast.makeText(AccelSecondary.this, "The device does NOT support Secondary Accel sensor", Toast.LENGTH_SHORT).show();
                AccelSecondary.this.finish();
            }

            sensorSecondary = mSensorList.get(secondAccelId);
    ...
    public void onSensorChanged(SensorEvent event)
    {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER)
        {
            ...
            if ((System.currentTimeMillis() - lastUiUpdateTime) > UI_UPDATE_RATE)
            {
                ...
                dbgLog(TAG, "sensor: secondary accelerometer" + ", x: " + fAccelerometerX + ", y: " + fAccelerometerY + ", z: " + fAccelerometerZ, 'd');
            }
        }
    ...
    protected void handleTestSpecificActions() throws CmdFailException, CmdPassException
    {
        ...
        if (strRxCmd.equalsIgnoreCase("GET_READING"))
        {
            ...
            strDataList.add(String.format("SECONDARY_ACCELEROMETER_X=" + TestUtils.round(fAccelerometerX, PRECISION)));
            strDataList.add(String.format("SECONDARY_ACCELEROMETER_Y=" + TestUtils.round(fAccelerometerY, PRECISION)));
            strDataList.add(String.format("SECONDARY_ACCELEROMETER_Z=" + TestUtils.round(fAccelerometerZ, PRECISION)));
            strDataList.add(String.format("MAX_SECONDARY_ACCELEROMETER_X=" + TestUtils.round(fMaxAccelerometerX, PRECISION)));
            strDataList.add(String.format("MAX_SECONDARY_ACCELEROMETER_Y=" + TestUtils.round(fMaxAccelerometerY, PRECISION)));
            strDataList.add(String.format("MAX_SECONDARY_ACCELEROMETER_Z=" + TestUtils.round(fMaxAccelerometerZ, PRECISION)));
            strDataList.add(String.format("MIN_SECONDARY_ACCELEROMETER_X=" + TestUtils.round(fMinAccelerometerX, PRECISION)));
            strDataList.add(String.format("MIN_SECONDARY_ACCELEROMETER_Y=" + TestUtils.round(fMinAccelerometerY, PRECISION)));
            strDataList.add(String.format("MIN_SECONDARY_ACCELEROMETER_Z=" + TestUtils.round(fMinAccelerometerZ, PRECISION)));
            strDataList.add(String.format("TOTAL_SECONDARY_ACCELERATION=" + TestUtils.round(fTotalAcceleration, PRECISION)));
    ...
    protected void printHelp()
    {
        ...
        strHelpList.add(TAG);
        strHelpList.add("");
        strHelpList.add("This function will read the 2nd Accelerometer");
        strHelpList.add("");
    ...
    public boolean onKeyDown(int keyCode, KeyEvent ev)
    {
        ...
        if ((keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) || (keyCode == KeyEvent.KEYCODE_VOLUME_UP))
        {
            ...
            {
                contentRecord("testresult.txt", "2nd ACCEL Test: PASS" + "\r\n", MODE_APPEND);
                logResults(TEST_PASS);
            }
            else
            {
                contentRecord("testresult.txt", "2nd ACCEL Test: FAILED" + "\r\n", MODE_APPEND);
    ...
    private void logResults(String passFail)
    {
        ...
        testResultName.add("SECONDARY_ACCELEROMETER_X");
        testResultName.add("SECONDARY_ACCELEROMETER_Y");
        testResultName.add("SECONDARY_ACCELEROMETER_Z");
        testResultName.add("MAX_SECONDARY_ACCELEROMETER_X");
        testResultName.add("MAX_SECONDARY_ACCELEROMETER_Y");
        testResultName.add("MAX_SECONDARY_ACCELEROMETER_Z");
        testResultName.add("MIN_SECONDARY_ACCELEROMETER_X");
        testResultName.add("MIN_SECONDARY_ACCELEROMETER_Y");
        testResultName.add("MIN_SECONDARY_ACCELEROMETER_Z");
        testResultName.add("TOTAL_SECONDARY_ACCELERATION");
